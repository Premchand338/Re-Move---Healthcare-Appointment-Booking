      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 items-stretch">
          {MODALITIES_DATA.map((modality) => (
            <div
              key={modality.id}
              onClick={() => onOpenEvidence(modality)}
              className="bg-[#FAFAF8] hover:bg-white border-2 border-neutral-200 hover:border-neutral-950 hover:-translate-y-1 rounded-3xl p-6 real-shadow-xs hover:real-shadow-xl flex flex-col justify-between transition-all duration-150 group cursor-pointer"
            >
              <div>
                {/* Evidence Strength Badge */}
                <div className="flex items-center justify-between border-b border-neutral-200 pb-3 mb-4">
                  <span className="text-[10px] font-clinical-mono uppercase tracking-widest text-neutral-500 font-bold">
                    EVIDENCE TIER
                  </span>
                  <span
                    className={`text-[10px] font-clinical-mono font-black px-2.5 py-0.5 rounded-full border ${getEvidenceColor(
                ...
                {/* Modality Title */}
                <h3 className="font-editorial-serif text-xl font-bold text-neutral-900 tracking-tight leading-snug">
                  {modality.title}
                </h3>

                <p className="text-xs font-clinical-mono text-emerald-800 font-bold uppercase tracking-wider mt-1.5 leading-snug">
                  {modality.subtitle}
                </p>

                <p className="mt-3 text-xs sm:text-sm text-neutral-600 font-editorial-sans leading-relaxed">
                  {modality.description}
                </p>
              </div>

              {/* Action Link with High Affordance */}
              <div className="mt-6 pt-4 border-t border-neutral-200">
                <button
                  type="button"
                  onClick={(e) => {
                    e.stopPropagation();
                    onOpenEvidence(modality);
                  }}
                  className="w-full inline-flex items-center justify-between text-xs font-clinical-mono font-black text-neutral-950 py-2.5 px-3.5 rounded-xl bg-white border-2 border-neutral-200 group-hover:border-neutral-950 transition-all duration-150 uppercase tracking-wider cursor-pointer active:scale-95"
                >
                  <span>Review Clinical Trial Data</span>
                  <ArrowUpRight
                    size={14}
                    className="transition-transform duration-150 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 text-neutral-900"
                  />
                </button>
              </div>
            </div>
